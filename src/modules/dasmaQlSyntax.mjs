import { dasmaQlProcessor } from "../processors/dasmaQlProcessor.js";
import * as dasmaQlParser from "../parsers/dasmaQlParser.cjs";
...
class DasmaQlSyntax {
  ...
  #configureSyntaxProcessor(options) {
    ...
    if (options.validFields.length) {
      syntaxProcessor.validateTerm = (term) =>
        this.#validateTerm(term, options.validFields);
      syntaxProcessor.validateOrder = (order) =>
        this.#validateOrder(order, options.validFields);
    }
