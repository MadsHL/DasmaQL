...
const levenshteinSearchSort = (elements, search) => {
  ...
  return elements
    ...
    .map((element) => ({
      ...element,
      distance:
        levenshteinDistance(element.label, searchLower) -
        (element.label.length - searchLower.length),
    }))
    ...
    .sort((a, b) => {
      if (a.distance === b.distance) {
        return getElementLabelLower(a.element).localeCompare(
          getElementLabelLower(b.element),
        );
      }
...
export { levenshteinDistance, levenshteinSearchSort };
