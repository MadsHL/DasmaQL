...
class DasmaQlHighlighter {
  ...
  highlight(qlString) {
    const tokens = dasmaQlSuggestionParser.parse(
      qlString,
  ...
  #evaluateToken(token, evaluation) {
    ...
    switch (token.type) {
      case "parameter":
        newEvaluation = this.#parser(token.field, evaluation, "field");
  ...
  #parser(token, evaluation, clazz) {
    ...
    const cssError = this.#validate(token)
      ? ""
      : `${this.options.prefixClass}-error`;
    ...
    const highlighted =
      qlString.substring(0, start) + replacement + qlString.substring(end);
  ...
  #validateField(token) {
    ...
    if (this.options?.validFields?.length > 0 && token?.input) {
      valid &= this.options?.validFields?.includes(
        this.#cleanString(token.input),
      );
    }
    return valid;
  }
